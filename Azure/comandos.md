...
disponíveis na sua conta do Azure.Comando 05:
...
Comando 14:New-AzResourceGroup -Name "NovoGrupoDeRecursos" -Location
...
Comando 19:
New-AzVM -ResourceGroupName "MeuGrupoDeRecursos" -Name
"NovaVM" -Image UbuntuLTS -Size Standard_B2s -Location "East
US"
...
Comando 20:
$vmConfig = New-AzVMConfig -VMName "NovaVM" -VMSize
Standard_D2s_v3
New-AzVM -ResourceGroupName "MeuGrupoDeRecursos"
-Location "West Europe" -VM $vmConfig
...
"MeuGrupoDeRecursos" e localização "West Europe".Comando 21:
$vmParams = @{
...
Comando 24:$vm = Get-AzVM -ResourceGroupName "MeuGrupoDeRecursos"
-Name "MinhaVM"
Start-AzVM -VM $vm
...
Comando 27:$vmsToStop = Get-AzVM -ResourceGroupName
...
rapidamente a VM.
Comando 30:$vm = Get-AzVM -ResourceGroupName "MeuGrupoDeRecursos"
-Name "MinhaVM"
Restart-AzVM -VM $vm
Esse comando obtém a máquina virtual chamada "MinhaVM" no
...
-TagValue "Finance"Esse comando retorna todas as contas de armazenamento com o
...
Comando 45:New-AzAppServicePlan -ResourceGroupName
"MeuGrupoDeRecursos" -Name "NovoPlano" -Location "West
...
classificados em ordem decrescente pelo nome.Comando 49:
...
Comando 55:New-AzSQLServer -ResourceGroupName "MeuGrupoDeRecursos"
...
Comando 64:Get-AzVirtualNetwork -ResourceGroupName
"MeuGrupoDeRecursos"
Esse comando retorna informações detalhadas sobre todas as
...
Comando 70:Get-AzNetworkInterface -ResourceGroupName
"MeuGrupoDeRecursos"
...
"10.0.0.4"Esse comando cria uma nova interface de rede com o nome
...
possuem a tag "Environment" com o valor "Production".Comando 79:
...
New-AzPublicIpAddress @ipParamsEsse comando cria um novo endereço IP público usando um
hashtable para fornecer os parâmetros, permitindo maior
...
dados armazenados.
Comando 84:Get-AzStorageAccountKey -ResourceGroupName
"MeuGrupoDeRecursos" -Name "MinhaContaDeArmazenamento" |
...
contêiner de blob.
Comando 87:Set-AzStorageBlobContent -Container "meucontainer" -Blob
...
tag "Environment" com o valor "Production".Comando 91:
...
São 127 comandos para você utilizar, esses são os principais que
utilizamos no Azure no dia a dia.
Mas não adianta saber exatamente qual comando rodar, se não
temos o conhecimento dos recursos do Azure.
Se você colocou como meta profissional se tornar um arquiteto em
Azure…
Em outras palavras, ser um VERDADEIRO Especialista em
Azure…
Eu tenho a minha Formação Azure Architect. O objetivo dessa
Formação é transformar alunos em verdadeiros especialistas e
arquitetos em ambiente com Azure.
Um ano de formação, onde você vai aprender a desenhar, planejar
e implementar soluções robustas no Azure.
Entre em contato com a minha equipe no Whatsapp para saber
todos os detalhes da Formação Azure Architect, e vamos começar
a transformar sua carreira imediatamente.
https://bit.ly/3MepurS
